...
class ProfileScreen extends StatelessWidget {
        ...
        child: Column(
          children: [
            ...
            _buildMenuItem(
              ...
              trailingText: user?.phone?.isNotEmpty == true ? user!.phone : 'Edit',
            ...
            Container(
              ...
              child: ListTile(
                ...
                trailing: const Icon(Icons.chevron_right_rounded, color: AppTheme.error, size: 18),
              ),
            ),
  ...
  static Widget _buildSectionLabel(String label) {
    return Padding(
      ...
      child: Text(
        label,
        style: const TextStyle(color: AppTheme.textSecondary, fontSize: 11.5, fontWeight: FontWeight.w700, letterSpacing: 0.5),
      ),
    );
  }
  ...
  static Widget _buildMenuItem({
    ...
    return Container(
      ...
      child: ListTile(
        ...
        title: Text(title, style: const TextStyle(color: AppTheme.textPrimary, fontWeight: FontWeight.w600, fontSize: 13.5)),
        trailing: Row(
          mainAxisSize: MainAxisSize.min,
          children: [
            if (trailingText != null)
              Text(trailingText, style: const TextStyle(color: AppTheme.textSecondary, fontSize: 11.5)),
    ...
    showDialog<void>(
      ...
      builder: (dialogCtx) => StatefulBuilder(
        builder: (_, setState) => AlertDialog(
          ...
          title: const Text('Edit Informasi Akun', style: TextStyle(color: AppTheme.textPrimary, fontWeight: FontWeight.bold, fontSize: 16)),
          content: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              TextField(controller: nameCtrl, decoration: const InputDecoration(labelText: 'Nama Lengkap', hintText: 'Masukkan nama...')),
              const SizedBox(height: 12),
              TextField(controller: phoneCtrl, keyboardType: TextInputType.phone, decoration: const InputDecoration(labelText: 'Nomor WhatsApp', hintText: '0812...')),
            ],
          ),
          actions: [
            ...
            ElevatedButton(
              onPressed: isLoading ? null : () async {
                ...
                final ok = await auth.updateProfile(name: nameCtrl.text.trim(), phone: phoneCtrl.text.trim());
                setState(() => isLoading = false);
                if (dialogCtx.mounted) Navigator.pop(dialogCtx);
                if (context.mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(ok ? 'Profil berhasil diperbarui!' : (auth.error ?? 'Gagal memperbarui profil.'))));
                }
              },
      ...
      builder: (dialogCtx) => StatefulBuilder(
        builder: (_, setState) => AlertDialog(
          ...
          content: Column(
            ...
            children: [
              const Text('Masukkan kode kupon voucher untuk menambah saldo token kamu.', style: TextStyle(color: AppTheme.textSecondary, fontSize: 12)),
        ...
        builder: (_, setState) => AlertDialog(
          ...
          content: tab == 0
              ...
              : Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text('Link reset password akan dikirimkan ke:\n${auth.user?.email ?? 'email terdaftar'}',
                  ...
                  Container(
                    ...
                    child: const Row(children: [
                      ...
                      SizedBox(width: 8),
                      Expanded(child: Text('Silakan cek inbox email kamu setelah mengirim link reset.', style: TextStyle(color: AppTheme.primary, fontSize: 11))),
                    ]),
                  ),
                ]),
          actions: [
            ...
            ElevatedButton(
              onPressed: isLoading ? null : () async {
                if (tab == 0) {
                  ...
                  if (dialogCtx.mounted) Navigator.pop(dialogCtx);
                  if (context.mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Link reset password dikirim ke ${auth.user?.email ?? 'email'}. Cek inbox kamu.')));
                  }
                }
              },
              child: isLoading
  ...
  void _showMembershipDialog(BuildContext context, AuthProvider auth) {
    showDialog<void>(
      ...
      builder: (dialogCtx) => AlertDialog(
        backgroundColor: AppTheme.bgSurface,
        title: const Text('Paket & Membership', style: TextStyle(color: AppTheme.textPrimary, fontWeight: FontWeight.bold, fontSize: 16)),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
                ...
                children: [
                  ...
                  Expanded(
                    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text(auth.user?.plan ?? 'Free Plan', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 16)),
                      const SizedBox(height: 2),
                      Text('Saldo Token: ${auth.tokenBalance}', style: const TextStyle(color: Colors.white70, fontSize: 12)),
                    ]),
                  ),
                ],
              ),
            ...
            const SizedBox(height: 14),
            const Text('Keuntungan Paket:', style: TextStyle(color: AppTheme.textPrimary, fontWeight: FontWeight.bold, fontSize: 12.5)),
            const SizedBox(height: 6),
            const Text('• AI Chat (GPT-4o, Gemini, Claude, DeepSeek, Qwen)\n• AI Writer + Storage 500MB\n• Paraphrase AI & Plagiarism Check\n• Prioritas akses model terbaru',
  ...
  void _showThemeDialog(BuildContext context, ThemeProvider theme) {
    showDialog<void>(
      ...
      builder: (dialogCtx) => AlertDialog(
        backgroundColor: AppTheme.bgSurface,
        title: const Text('Mode Tampilan', style: TextStyle(color: AppTheme.textPrimary, fontWeight: FontWeight.bold, fontSize: 16)),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            _buildThemeOption(context, dialogCtx, theme, '☀️  Mode Terang (Light)', ThemeMode.light, !theme.isDarkMode),
            const SizedBox(height: 6),
            _buildThemeOption(context, dialogCtx, theme, '🌙  Mode Gelap (Dark)', ThemeMode.dark, theme.isDarkMode),
          ],
        ),
      ),
    );
  ...
  static Widget _buildThemeOption(BuildContext context, BuildContext dialogCtx, ThemeProvider theme, String label, ThemeMode mode, bool selected) {
    return InkWell(
      ...
      child: Container(
        ...
        child: Row(
          children: [
            Expanded(child: Text(label, style: TextStyle(color: selected ? AppTheme.primary : AppTheme.textPrimary, fontWeight: selected ? FontWeight.bold : FontWeight.normal))),
